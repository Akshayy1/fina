...
function next_1() {
   if (count == 0) {
      ...
      if (v == 0) {
         count++;
         obj.fname = page_oneinput[v].value;
         v++;
         obj.lname = page_oneinput[v].value;
         v++;
   ...
   //  endone
      ...
      let allform = document.getElementsByClassName("tobeadded")
      let school = document.getElementsByClassName("school")
      let passing = document.getElementsByClassName("passing")
      let degree = document.getElementsByClassName("degree")
   debugger
      for (let i = 0; i < allform.length; i++) {
         if (school[i].value !== "" && passing[i].value !== "" && degree[i].value !== "") {
            let educationObj = {};
            educationObj[school[i].id] = school[i].value;
            educationObj[passing[i].id] = passing[i].value;
            educationObj[degree[i].id] = degree[i].value;
            obj.education[i] = educationObj;
      }
...
function adds() {
   let backtick = `<div class="tobeadded">
   <div class="information ">
       <label for="school">School Name</label>
       <input type="text" id="school" class="dataone school">
       <p id="error11"></p>
   </div>
   <div class="information">
       <label for="passing">Passing Year</label>
       <input type="number" id="passing" class="dataone passing">
       <p id="error22"></p>
   </div>
   <div class="information">
       <label for="degree">Degree</label>
       <input type="text" id="degree" class="dataone degree">
       <p id="error33"></p>
   </div>
  </div>`
